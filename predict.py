...
# ---- constants -------------------------------------------------------------
WORKFLOW_JSON = "chroma_workflow.json"
...
class Predictor(BasePredictor):
    def setup(self):
        ...
        clip_path = os.path.join(clip_dir, "t5xxl_fp8_e4m3fn.safetensors")
        if not os.path.exists(clip_path):
            print(f"Downloading CLIP to {clip_path}...")
            try:
                CLIP_URL = (
                    "https://huggingface.co/comfyanonymous/flux_text_encoders/"
                    "resolve/main/t5xxl_fp16.safetensors"  # 4.9 GB FP-8
                )
                subprocess.check_call(
                    ["wget", "--no-verbose", CLIP_URL, "-O", clip_path]
                )
